...
const SUPABASE_KEY =
  "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6ImttZnByeGJmYWZicXVpbGdjaHR2Iiwicm9sZSI6ImFub24iLCJpYXQiOjE3NzI3ODUxODYsImV4cCI6MjA4ODM2MTE4Nn0.09WBWLQMfo_rYhN9MO2fukbd-YLNZMghFf0dCdjs5Jc";

async function storeLoad(key) {
  try {
    const res = await fetch(
      `${SUPABASE_URL}/rest/v1/kv_store?key=eq.${key}&select=value`,
      {
        headers: {
          apikey: SUPABASE_KEY,
...
const C = {
  ...
  border: "#30363d",
  accent: "#f97316",
  accentLow: "rgba(249,115,22,0.13)",
  green: "#22c55e",
  greenLow: "rgba(34,197,94,0.11)",
  blue: "#3b82f6",
  blueLow: "rgba(59,130,246,0.13)",
  text: "#e6edf3",
  muted: "#8b949e",
  red: "#f85149",
};
...
function parseCoords(raw) {
  ...
  const dms =
    /(\d{1,3})[°d]\s*(\d{1,2})['′m]\s*([\d.]+)["″s]?\s*([NS])[,\s]+(\d{1,3})[°d]\s*(\d{1,2})['′m]\s*([\d.]+)["″s]?\s*([EW])/i;
  m = s.match(dms);
  if (m) {
  ...
  return (
    <span
      style={{
        ...
        border: `1px solid ${
          done ? C.green + "55" : blue ? C.blue + "55" : C.border
        }`,
      }}
    >
      {label}
    ...
    <div style={{ display: "flex", flexDirection: "column", gap: 4 }}>
      ...
      {status === "err" && (
        <div style={{ fontSize: 11, color: C.red }}>
          ⚠ Format not recognised — try: 51.5074, -0.1278
        </div>
      )}
      {status === "shortlink_fail" && (
        <div style={{ fontSize: 11, color: C.red }}>
          ⚠ Couldn't extract coordinates from this link. Open it in your browser
          and copy the URL from the address bar instead.
        </div>
      )}
      {status === null && (
        <div style={{ fontSize: 10, color: C.muted, lineHeight: 1.5 }}>
          Accepts:{" "}
          <span style={{ fontFamily: "monospace" }}>51.5074, -0.1278</span> ·
          DMS · Google Maps link
        </div>
      )}
    </div>
  );
          ...
          {filtered.map((s, i) => (
            <div
              ...
              style={{
                ...
                fontSize: 13,
                cursor: "pointer",
                background: i === hi ? C.accentLow : "transparent",
                color: i === hi ? C.accent : C.text,
                borderBottom:
      ...
      {showDrop && (
        ...
        >
          {filtered.map((entry, i) => (
            ...
            >
              <div
                style={{ color: i === hi ? C.accent : C.text, fontSize: 13 }}
              >
                {entry.name}
              </div>
              <div
              ...
              >
                📍 {entry.coords.lat}, {entry.coords.lng}
              </div>
            </div>
          ))}
        </div>
...
function RouteMap({ stops }) {
  ...
  useEffect(() => {
    ...
    validStops.forEach((stop, i) => {
      const lat = parseFloat(stop.coords.lat),
        lng = parseFloat(stop.coords.lng);
      ...
      marker.bindPopup(
        `<b>${i + 1}. ${stop.name || stop.address || "Stop"}</b>`
      );
    });
...
function buildGoogleMapsUrl(stops) {
  ...
  if (validStops.length === 1) {
    return `https://www.google.com/maps/search/?api=1&query=${validStops[0].coords.lat},${validStops[0].coords.lng}`;
  }
  const origin = `${validStops[0].coords.lat},${validStops[0].coords.lng}`;
  const destination = `${validStops[validStops.length - 1].coords.lat},${
    validStops[validStops.length - 1].coords.lng
  }`;
  const middle = validStops.slice(1, -1).slice(0, 8);
  const waypointParam =
    middle.length > 0
      ? `&waypoints=${middle
  ...
  return (
    ...
    >
      ...
      >
        ...
        <a
          href={`https://maps.google.com/?q=${gq}`}
          target="_blank"
          rel="noreferrer"
          style={{
            display: "flex",
    ...
    >
      ...
      >
        ...
        <InputField
          ...
          value={f.name}
          onChange={(v) => s("name", v)}
          placeholder="e.g. Home, Office, Depot"
        />
        <InputField
          label="Address"
          value={f.address}
      ...
      >
        <div>
          ...
          <div style={{ color: C.muted, fontSize: 12, marginTop: 2 }}>
            Home base, depot, offices — route anchors
          </div>
        </div>
      ...
      {places.length === 0 && (
        <div style={{ textAlign: "center", padding: "64px 0", color: C.muted }}>
          ...
          <div style={{ fontSize: 13, marginTop: 4 }}>
            Add your home base or common start/end points
          </div>
        </div>
      )}
        ...
        return (
          ...
          >
            ...
            <div style={{ display: "flex", gap: 6 }}>
              <Btn onClick={() => setModal(pl)} variant="secondary" small>
                <Edit2 size={11} />
              </Btn>
              <Btn onClick={() => onDelete(pl.id)} variant="danger" small>
                <Trash2 size={11} />
              </Btn>
            </div>
          </div>
...
function JobModal({ job, vocab, addrBook, onSave, onClose }) {
  ...
  const s = (k, v) => setF((p) => ({ ...p, [k]: v }));
  const sk = (k, v) =>
    setF((p) => ({ ...p, completion: { ...p.completion, [k]: v } }));
  ...
  const handleAddressPick = (entry) => {
    setF((p) => ({
      ...p,
      name: entry.name,
      coords: { lat: entry.coords.lat, lng: entry.coords.lng },
    }));
  };
      ...
      >
        ...
        <Section label="Location">
          ...
          <Grid2>
            ...
            <AutoField
              label="Company"
              value={f.company}
              onChange={(v) => s("company", v)}
              placeholder="Client name"
              suggestions={V("company")}
            />
          </Grid2>
          <CoordField
            coords={f.coords}
        ...
        <Section label="Survey Details">
          <AutoField
            label="Survey type"
            value={f.surveyType}
            onChange={(v) => s("surveyType", v)}
            placeholder="e.g. CCTV drain survey"
            suggestions={V("surveyType")}
          />
          <Grid2>
            <AutoField
              label="Diameter (mm)"
              value={f.diameter}
              onChange={(v) => s("diameter", v)}
              placeholder="150"
              suggestions={V("diameter")}
            />
            <AutoField
              label="Approx. length (m)"
              value={f.approxLength}
              onChange={(v) => s("approxLength", v)}
              placeholder="80"
              suggestions={V("approxLength")}
            />
          </Grid2>
          <Grid2>
            <AutoField
              label="Branch diam. (mm)"
              value={f.branchDiameter}
              onChange={(v) => s("branchDiameter", v)}
              placeholder="100"
              suggestions={V("branchDiameter")}
            />
            <AutoField
              label="Volume (m³)"
              value={f.volume}
                ...
                return (
                  <label
                    ...
                    style={{
                      ...
                      borderRadius: 9,
                      cursor: "pointer",
                      background: done ? C.greenLow : C.surfaceHigh,
                  ...
                  >
                    <input
                      ...
                      checked={done}
                      style={{ display: "none" }}
                      onChange={(e) => sk(key, e.target.checked)}
                    />
                    <Checkbox done={done} />
                    <span
                      style={{
...
function JobCard({ job, onEdit, onDelete }) {
  const [open, setOpen] = useState(false);
  const doneCount = COMPLETION_FIELDS.filter(
    (f) => job.completion?.[f.key]
  ).length;
  ...
  return (
    <div
      style={{
        background: C.surface,
        border: `1px solid ${allDone ? C.green + "44" : C.border}`,
        borderRadius: 14,
        overflow: "hidden",
      }}
    >
      ...
      >
        ...
        <div style={{ flex: 1, minWidth: 0 }}>
          ...
          >
            ...
            <Pill
              label={`${doneCount}/${COMPLETION_FIELDS.length}`}
              done={allDone}
            />
      ...
      {open && (
        ...
        >
          ...
          <div style={{ display: "flex", gap: 8 }}>
            <Btn onClick={() => onEdit(job)} variant="secondary" small>
              <Edit2 size={12} /> Edit
            </Btn>
            <Btn onClick={() => onDelete(job.id)} variant="danger" small>
    ...
    <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
      ...
      >
        <div>
          ...
          <div style={{ color: C.muted, fontSize: 12, marginTop: 2 }}>
            {jobs.length} total ·{" "}
            {jobs.filter((j) => j.completion?.payment).length} invoiced
          </div>
        </div>
        <Btn onClick={() => setModal("new")}>
      ...
      {jobs.length === 0 && (
        <div style={{ textAlign: "center", padding: "64px 0", color: C.muted }}>
          ...
          <div style={{ fontSize: 13, marginTop: 4 }}>
            Tap New Job to create your first survey
          </div>
        </div>
      )}
...
function RoutingTab({ jobs, places, vocab, addrBook, onUpdateJob }) {
  ...
  const removePlace = (uid) =>
    setSelPlaces((p) => p.filter((e) => e.uid !== uid));
  ...
  const togglePin = (pinName, uid) => {
    ...
    // clear this uid from any other pin first
    PINS.forEach((p) => {
      if (p !== pinName && pinState[p] === uid) pinSetters[p](null);
    });
    const newVal = current === uid ? null : uid;
    setter(newVal);
    // when Start is set, auto-fill End with the same stop (if End not already set)
    if (pinName === "start" && newVal && !pinEndId) setPinEndId(newVal);
    // when Start is cleared, clear End too if it matched
  ...
  const allItems = [
    ...selPlaces
      ...
      .filter(Boolean),
    ...jobs
      .filter((j) => selJobs.has(j.id))
      .map((j) => ({ ...j, _type: "job" })),
  ];

  const optimise = () => {
    setRoute(tsp(allItems, pinStartId, pinFirstId, pinLastId, pinEndId));
    setStop(0);
    setPhase("run");
  };
  ...
  if (phase === "run" && route.length > 0) {
    ...
    const fDone = route.filter(
      (i) => i._type === "job" && i.completion?.fieldWork
    ).length;
    ...
    return (
      <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
        ...
        <div>
          ...
          >
            <span>
              Stop {stop + 1} of {route.length}
            </span>
            <span>
              {fDone}/{nJobs} field done
            </span>
          </div>
          <div
            style={{ height: 4, background: C.surfaceHigh, borderRadius: 99 }}
          >
            <div
              style={{
                height: "100%",
        ...
        <div
          style={{
            background: C.surface,
            border: `2px solid ${isJob ? C.accent : C.blue}`,
            borderRadius: 18,
            padding: 18,
            display: "flex",
            flexDirection: "column",
        ...
        >
          <div style={{ display: "flex", gap: 10, alignItems: "flex-start" }}>
            <div
              style={{
                ...
                height: 32,
                borderRadius: "50%",
                background: isJob ? C.accent : C.blue,
                color: "#fff",
                fontWeight: 800,
                fontSize: 13,
                display: "flex",
            ...
            <div>
              ...
              {isJob && item.surveyType && (
                <div style={{ color: C.accent, fontSize: 12, marginTop: 3 }}>
                  {item.surveyType}
                </div>
              )}
              {isJob && (item.diameter || item.approxLength || item.volume) && (
                <div style={{ color: C.muted, fontSize: 12, marginTop: 3 }}>
                  {[
                    ...
                    item.approxLength && `~${item.approxLength}m`,
          ...
          <Btn onClick={() => setNavModal(item)} full>
          ...
          {isJob && (
            ...
            >
              <Btn variant="secondary" onClick={() => setEditModal(item)}>
              ...
              <Btn
                ...
                onClick={() => {
                  onUpdateJob(item.id, {
                    completion: { ...item.completion, fieldWork: true },
                  });
                  if (stop < route.length - 1) setStop((s) => s + 1);
                }}
              >
                <Check size={12} /> Field Done
              </Btn>
            </div>
          )}
        </div>
        ...
        >
          <Btn
            variant="secondary"
            disabled={stop === 0}
            onClick={() => setStop((s) => s - 1)}
          >
            <ChevronLeft size={14} /> Prev
          </Btn>
          <Btn
            variant="secondary"
            disabled={stop === route.length - 1}
            onClick={() => setStop((s) => s + 1)}
          >
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 6 }}>
          ...
          {route.map((it, i) => {
            const active = i === stop,
              done = it._type === "job" && !!it.completion?.fieldWork;
              ...
              >
                <div
                  style={{
                    ...
                    alignItems: "center",
                    justifyContent: "center",
                    background: done ? C.green : active ? col : C.surfaceHigh,
                    fontSize: 10,
                    fontWeight: 800,
                    color: done || active ? (done ? "#000" : "#fff") : C.muted,
                  }}
                >
                ...
                {[pinStartId, pinFirstId, pinLastId, pinEndId].includes(
                  it.id
                ) && (
        ...
        {navModal && (
          <NavModal item={navModal} onClose={() => setNavModal(null)} />
        )}
        {editModal && (
          <JobModal
            job={editModal}
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
      <div>
        ...
        <div style={{ color: C.muted, fontSize: 12, marginTop: 2 }}>
          Pick places &amp; jobs · pin start/end · optimise
        </div>
      </div>
      ...
      {places.length > 0 ? (
          ...
          {places.map((pl) => {
            ...
            return (
              <div
                ...
                style={{
                  background: C.surface,
                  border: `1px solid ${count > 0 ? C.blue : C.border}`,
                  borderRadius: 12,
                  transition: "border-color .15s",
                }}
              >
                ...
                >
                  <div style={{ flex: 1, minWidth: 0 }}>
                    ...
                    {(pl.address || pl.city) && (
                      ...
                      >
                        {[pl.address, pl.city].filter(Boolean).join(", ")}
                      </div>
                    )}
                  </div>
                  <div
                  ...
                  >
                    {count > 0 && (
                      <button
                        onClick={() =>
                          removePlace(instances[instances.length - 1].uid)
                        }
                        style={{
                          width: 28,
                          height: 28,
                    ...
                    <button
                      onClick={() => addPlace(pl.id)}
                      style={{
                        width: 28,
                        height: 28,
                        borderRadius: 8,
                        background: count > 0 ? C.blueLow : C.surfaceHigh,
                        border: `1px solid ${count > 0 ? C.blue : C.border}`,
                        color: count > 0 ? C.blue : C.muted,
                        cursor: "pointer",
                        fontSize: 16,
                        fontWeight: 700,
                        display: "flex",
                        ...
                        >
                          ...
                          {[
                            ...
                            return (
                              <button
                                key={name}
                                onClick={() => togglePin(name, uid)}
                                title={title}
                                style={{
                                  background: active ? C.green : C.surfaceHigh,
                                  border: `1px solid ${
        ...
        >
          No places saved — add home base or depot in the{" "}
          <strong style={{ color: C.text }}>Places</strong> tab.
        </div>
      )}
      ...
      {jobs.length > 0 ? (
        <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
          <div style={{ display: "flex", alignItems: "center", gap: 6 }}>
            ...
            >
              <button
                onClick={() => setSelJobs(new Set(jobs.map((j) => j.id)))}
                style={{
                  background: "none",
                  border: "none",
                  color: C.muted,
                ...
                >
                  ...
                  <div style={{ flex: 1, minWidth: 0 }}>
                    ...
                    >
                      ...
                      {job.completion?.fieldWork && (
                        <Pill label="✓ field" done />
                      )}
                    </div>
                  </div>
                  {sel && (
        ...
        >
          No jobs yet — add them in the{" "}
          <strong style={{ color: C.text }}>Jobs</strong> tab.
        </div>
      )}
      ...
      {allItems.length > 0 && (
        ...
        >
          🟢 <strong style={{ color: C.text }}>Start</strong> &amp; 🏁{" "}
          <strong style={{ color: C.text }}>End</strong> are required · 1️⃣{" "}
          <strong style={{ color: C.text }}>First</strong> &amp; 🔚{" "}
          <strong style={{ color: C.text }}>Last</strong> are optional
          <br />
      ...
      {noCoords.length > 0 && (
        ...
        >
          ⚠ {noCoords.length} stop{noCoords.length > 1 ? "s" : ""} without
          coordinates — optimisation will be approximate.
        </div>
      )}
      ...
      <Btn
        onClick={optimise}
        disabled={allItems.length < 1 || !pinStartId || !pinEndId}
        full
      >
        <RotateCw size={14} /> Optimise & Start Run ({allItems.length} stop
        {allItems.length !== 1 ? "s" : ""})
      </Btn>
    </div>
  );
...
function GeoJSONImporter({ onImportVocab, onImportAddresses, onClose }) {
      ...
      try {
        ...
        keyList.forEach((k) => {
          const vals = [
            ...new Set(
              geo.features
                .map((f) => (f.properties?.[k] ?? "").toString().trim())
                .filter(Boolean)
            ),
          ].slice(0, 5);
          sampleMap[k] = vals;
        ...
        setError(
          "Couldn't read file — make sure it's a valid GeoJSON export from QGIS."
        );
      }
    };
  ...
  const doImport = () => {
    ...
    const additions = {};
    APP_VOCAB_FIELDS.forEach((af) => {
      additions[af.key] = new Set();
    });
    ...
    const countMap = {};
    Object.entries(additions).forEach(([k, s]) => {
      countMap[k] = s.size;
    });
    onImportVocab(
      Object.fromEntries(Object.entries(additions).map(([k, s]) => [k, [...s]]))
    );
    setCounts(countMap);
    setStep("done");
  };
      ...
      >
        ...
        {step === "upload" && (
          <>
            ...
            >
              Export your QGIS layer as{" "}
              <strong style={{ color: C.text }}>GeoJSON</strong> (Layer → Export
              → Save Features As → Format: GeoJSON). Then pick the file below.
            </p>
            <label
              style={{
                display: "flex",
        ...
        {step === "map" && (
          <>
            ...
            >
              Map your QGIS fields to app vocabulary fields. Unneeded fields set
              to <em>Ignore</em>.
            </p>
            <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
              {fields.map((geoKey) => (
                ...
                >
                  ...
                  >
                    ...
                    <select
                      ...
                      onChange={(e) =>
                        setMapping((m) => ({ ...m, [geoKey]: e.target.value }))
                      }
                      style={{
                        background: C.surface,
                        border: `1px solid ${C.border}`,
        ...
        {step === "done" && (
          <>
            ...
            <Btn onClick={onClose} full variant="secondary">
              Done
            </Btn>
          </>
        )}
...
function VocabEditor({ vocab, onSave, onClose }) {
  const [local, setLocal] = useState(() =>
    Object.fromEntries(
      APP_VOCAB_FIELDS.map((f) => [f.key, (vocab[f.key] ?? []).join("\n")])
    )
  );
  ...
  return (
    ...
    >
      ...
      >
            ...
            <button
              ...
              style={{
                ...
                borderBottom: `2px solid ${
                  tab === f.key ? C.accent : "transparent"
                }`,
                color: tab === f.key ? C.accent : C.muted,
                fontWeight: 700,
                fontSize: 12,
                whiteSpace: "nowrap",
              }}
        ...
        >
          <div style={{ fontSize: 11, color: C.muted }}>
            One value per line · {lineCount} entries
          </div>
          <textarea
            value={local[tab]}
...
function SyncTab({
  ...
  const decode = (str) =>
    JSON.parse(decodeURIComponent(escape(atob(str.trim()))));
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
      <div>
        ...
        <div style={{ color: C.muted, fontSize: 12, marginTop: 2 }}>
          Export your data as a code · share via WhatsApp · import on another
          phone
        </div>
      </div>
      ...
      >
        ...
        <p style={{ color: C.muted, fontSize: 13, margin: 0, lineHeight: 1.5 }}>
          Encodes all jobs, places, address book and vocabulary into a single
          code and copies it to your clipboard. Paste it into WhatsApp and send
          to the other phone.
        </p>
        <Btn
          onClick={handleExport}
          variant={exportDone ? "green" : "primary"}
          full
        >
          {exportDone ? (
            <>
      ...
      >
        ...
        <p style={{ color: C.muted, fontSize: 13, margin: 0, lineHeight: 1.5 }}>
          Paste the code received from another phone.{" "}
          ...
          <textarea
            ...
            onFocus={async () => {
              ...
              try {
                ...
                if (t?.trim()) setImportText(t.trim());
              } catch {}
            }}
        ...
        {importState === "err" && (
          <div style={{ fontSize: 11, color: C.red }}>
            ⚠ Invalid code — make sure you copied the full export text.
          </div>
        )}
        {importState === "ok" && (
      ...
      {confirmOpen && (
        ...
        >
          ...
          >
            ...
            >
              This will overwrite your current {stats.jobs} jobs, {stats.places}{" "}
              places and all vocabulary. This cannot be undone.
            </p>
            <div
              style={{
                display: "grid",
            ...
            >
              ...
              <Btn variant="danger" onClick={confirmImport}>
                Replace
              </Btn>
            </div>
          </div>
      ...
      >
        ...
        <p style={{ color: C.muted, fontSize: 13, margin: 0, lineHeight: 1.5 }}>
          Load a GeoJSON export from QGIS to populate autocomplete lists for
          city, company, survey type, diameter and more.
        </p>
        <Btn onClick={() => setGeoOpen(true)} variant="secondary" full>
          🗺 Import QGIS Vocabulary
        </Btn>
      </div>
      ...
      >
        ...
        <p style={{ color: C.muted, fontSize: 13, margin: 0, lineHeight: 1.5 }}>
          View and edit the autocomplete lists for survey type, company, city,
          diameter and more.
        </p>
        <Btn onClick={() => setVocabOpen(true)} variant="secondary" full>
          ✏️ Edit Vocabulary
        </Btn>
      </div>
...
function App() {
  ...
  useEffect(() => {
    Promise.all([
      storeLoad("srp6-jobs"),
      storeLoad("srp6-places"),
      storeLoad("srp6-vocab"),
      storeLoad("srp6-addr"),
    ]).then(([j, p, v, a]) => {
      if (j) setJobs(j);
      if (p) setPlaces(p);
  ...
  useEffect(() => {
    if (loaded) storeSave("srp6-jobs", jobs);
  }, [jobs, loaded]);
  useEffect(() => {
    if (loaded) storeSave("srp6-places", places);
  }, [places, loaded]);
  useEffect(() => {
    if (loaded) storeSave("srp6-vocab", vocab);
  }, [vocab, loaded]);
  useEffect(() => {
    if (loaded) storeSave("srp6-addr", addrBook);
  }, [addrBook, loaded]);
  ...
  const deleteJob = useCallback(
    (id) => setJobs((p) => p.filter((j) => j.id !== id)),
    []
  );
  const addPlace = useCallback((pl) => setPlaces((p) => [...p, pl]), []);
  const updatePlace = useCallback(
    (id, u) =>
      setPlaces((p) => p.map((pl) => (pl.id === id ? { ...pl, ...u } : pl))),
    []
  );
  const deletePlace = useCallback(
    (id) => setPlaces((p) => p.filter((pl) => pl.id !== id)),
    []
  );
  ...
  return (
    ...
    >
      ...
      >
        <div
          style={{ maxWidth: 540, margin: "0 auto", padding: "14px 16px 0" }}
        >
          <div
            style={{
              display: "flex",
      ...
      <div
        style={{ maxWidth: 540, margin: "0 auto", padding: "16px 16px 70px" }}
      >
        {tab === "jobs" && (
          <JobsTab
            jobs={jobs}
        ...
        {tab === "sync" && (
          <SyncTab
            ...
            addrBook={addrBook}
            onImport={importAll}
            onImportVocab={importVocab}
            onImportAddresses={importAddresses}
